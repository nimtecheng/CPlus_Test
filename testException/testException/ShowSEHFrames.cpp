...
{
    BOOL bVcExceptionHandler4 = pVCExcRec->handler == (FARPROC)_except_handler4;
    //VCµÄ_except_handler4º¯Êý
    if (bVcExceptionHandler4)
    {
        printf("Frame: %08X  Handler: %08X  Prev: %08X  Scopetable: %08X\n",
            pVCExcRec, pVCExcRec->handler, pVCExcRec->prev, (DWORD)pVCExcRec->scopetable ^ __security_cookie);
    }
    else
    {
    ...
    DWORD iAddr = (DWORD)pVCExcRec->scopetable ^ __security_cookie;
